...
const Contact = () => {
  ...
  const Submit=()=>{
    if(user.Name===''||user.Email===''||user.Message===''){
      toast.error('Please fill out first three forms')
      return
    }
  ...
  return (
    ...
    <div className="bg-secondary w-full md:mt-6 mt-8 flex justify-center items-center">
     <div className="w-full max-w-448 flex  items-center md:items-start  gap-3  py-20 flex-wrap  Divpadding flex-col">
      ...
      <input type="text" name='Name' value={user.Name} onChange={(e)=>handleUserForm(e)} className=" py-4 px-4 md:w-1/2 w-full md:max-w-128" placeholder="Name" />
      <input type="text" name='Email' value={user.Email} onChange={(e)=>handleUserForm(e)} className=" py-4 px-4 md:w-1/2 w-full md:max-w-128" placeholder="Email Address" />
     <textarea name="Message" onChange={(e)=>handleUserForm(e)} value={user.Message} id="" cols={30} rows={0} className=" py-4 px-4 w-full md:w-9/12 max-w-384 h-96" placeholder="Message"></textarea>
      <input type="text" name='Budget' value={user.Budget} onChange={(e)=>handleUserForm(e)} className=" py-4 px-4 md:w-1/2 w-full md:max-w-128" placeholder="Budget" />
      <button className="p-4 md:w-auto w-full px-8 bg-blue-600 hover:bg-blue-500 text-white" onClick={()=>Submit()}>Submit</button>
